...
import pandas as pd
df = pd.DataFrame(columns=['xyxy', 'confidence', 'class_id', 'tracker_id'])
...
ZONE_IN_POLYGONS = [
    ...
    np.array([[0, 0], [50000, 0], [50000, 50000], [0, 50000]]),
...
class VideoProcessor:
    def __init__(
        ...
        self.trace_annotator = sv.TraceAnnotator(
            color=COLORS, position=sv.Position.CENTER, trace_length=1000000, thickness=2
        )
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        df_temporal = pd.DataFrame(detections)

        #df_existente = pd.concat([df_existente, df_temporal], ignore_index=True)
        df_temporal.to_csv('ARCHIVOTRATADO.csv')
